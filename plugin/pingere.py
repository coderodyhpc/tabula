...
# ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
def renovatio_formas(canvas: QgsMapCanvas, servusII, pigmemtum='black', grossitudo='0.35') -> None:
    gdal_ds = transfiguro_formas(servusII)
    gdal_layer = gdal_ds.GetLayer(0)                 # type: ogr.Layer
    gdal_srs = gdal_layer.GetSpatialRef()            # type: osr.SpatialReference
    qgs_crs = get_qgis_crs(servusII.lex)

    genus_nomen = 'Domains (Vector)'

    unus = QgsProject.instance()                   # type: QgsProject
    radix = unus.layerTreeRoot()                   # type: QgsLayerTree
    genus = radix.findGroup(genus_nomen)           # type: QgsLayerTreeGroup
    if genus is None:
        genus = radix.insertGroup(0, genus_nomen)
        genus.setExpanded(False)
    else:
        genus.removeAllChildren()

    gdal_layer.ResetReading()
# This block might be for more than 1 domain        
    for idx, feature in enumerate(gdal_layer):
        if idx < 10:
            title = 'd00{}'.format(idx + 1)
        else:
            title = 'd0{}'.format(idx + 1)
        layer = QgsVectorLayer('Polygon?crs=' + qgs_crs.authid(), title, 'memory')
#        print ("LAYER at RENOVATIO ", layer)
#        sym = QgsFillSymbol.createSimple({
#            'outline_color': 'black' if idx == 0 else 'blue',
#            'style': 'no' # disable filling
#        })
        sym = QgsFillSymbol.createSimple({
            'outline_color': pigmemtum, 'outline_width': grossitudo, 'style': 'no' # disable filling
        })
        ...
        unus.addMapLayer(layer, False)
        genus.addLayer(layer)
#        print ("UNUS at RENOVATIO ", unus, " VALUES ", unus.instance().mapLayers().values())
#        print ("RADIX at RENOVATIO ", radix, radix.children())
#        print ("GENUS at RENOVATIO ", genus, genus.children())
#        for capa in unus.mapLayers().values():
#            if capa.name() == 'd001':
#                QgsProject.instance().removeMapLayers( [capa.id()] )
#        print ("2 UNUS at RENOVATIO ", unus, " VALUES ", unus.instance().mapLayers().values())
#        print ("2 GENUS at RENOVATIO ", genus, genus.children())
